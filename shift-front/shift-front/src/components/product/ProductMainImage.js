...
const ShopMain = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const cat1Res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/categories/1/products`);
      let cat1List = safeGetItems(cat1Res).filter(
        (item) => !latestIds.includes(item.productId)
      );
      cat1List = cat1List.slice(0, 4);

      const cat2Res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/categories/2/products`);
